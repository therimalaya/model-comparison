...
% Are we talking about ANOVA model again ??
...
%  {\tt msep}_{ijklms} = \mu + [{\tt p}_i + ({\tt R2})_j + {\tt relpos}_k + {\tt Method}_l + {\tt eta}_m + {\tt dataset}_s]^3 + \epsilon_{ijklms}
%\end{equation}
%I think maybe we don't need the model after all.

%where $\epsilon_{ijklms} \sim N(0, \sigma^2)$ and ${\tt dataset}_s \sim N(0, \sigma_{{\tt dataset}}^2)$

% From the fitted model, we found that the decay factor of eigenvalues ($\eta$) and the position of relevant components ($\mathcal{P}$) were not significant along with their interactions with the other factors. Since not all levels of interactions did not have significant effects on the minimum MSEP, a backward elimination was performed on the complete model to obtain a reduced model with ANOVA table:

% \begin{table}[ht]
%   \centering
%   \begin{tabularx}{0.8\textwidth}{Xrcrr}
%     \hline
%                 & F       & Df & Df.res & Pr($>$F) \\
%     \hline
...
%     \hline
%   \end{tabularx}
%   \caption{Analysis of Variance Table (Type II Wald F tests with Kenward-Roger df)}
...
% The Analysis of variance table (Table-\ref{tab:anova-table}) for the reduced
% model suggests that there is a significant third order interaction between the
% number of variables $(p)$, coefficient of determination $(\rho^2)$ and
% estimation method.


The effect of this third order interaction between $p$, $\rho^2$ and {\tt Methods}, which we see in Figure~\ref{fig:effect-plot}~(left), shows that the maximum likelihood based estimation methods, in our case, the envelope and the ordinary least squares, perform poorly on data sets with large number of variables and low $\rho^2$. Still, the performance of the envelope is better than ordinary least squares also in situations where $p\sim n$. The interaction plots also suggest that the Bayes PLS and ordinary PLS estimation methods are better and more stable on average than the two other methods.

Similarly, the effect of third order interaction between {\tt relpos}, $\eta$ and {\tt Method} in figure~\ref{fig:effect-plot}~(right) shows that OLS method gives higher prediction error than other methods but the effect of {\tt relpos} is small but notable for the envelope method. In order to understand these differences in detail, smallest MSEP (mean square error of prediction) over 5 replicated calibration sets are plotted for entire 32 designs. Number of latent components of predictor is plotted against MSEP for each estimation method. Component zero is added as an average of response in calibration set, i.e. for component zero, $\hat{y}_\text{test} = \bar{y}_\text{train}$, to see weather the methods are better than the average response.

To make it discrete, comments are organized in four groups:
\begin{enumerate*}[label = \alph*)]
\item \label{lst:g1} $p = 15$, $R^2 = 0.5$;
\item \label{lst:g2} $p = 15$, $R^2 = 0.9$;
\item \label{lst:g3} $p = 40$, $R^2 = 0.5$ and
\item \label{lst:g4} $p = 40$, $R^2 = 0.9$.
\end{enumerate*}
...
\end{figure}
In group~\ref{lst:g1}, with small number of variables $(p << n)$ and noisy data $(R^2 = 0.5)$, figure~\ref{fig:pred-error-15-1} shows that all the estimation methods has performed better than ordinary least squares for all designs in this group. There are some convergence issues with BayesPLS when eigenvalues of predictors decreases rapidly. However, the BayesPLS was able to achieve its minimum prediction error with fewer number of components. MSEP for BayesPLS is also smaller than other methods in most of the situations. If only few components are considered, convergence issue of BayesPLS doesn't make any problem. In addition, it seems that the envelope methods is performing well when there are few relevant components in initial positions.
...
Having few variables rich with information $(R^2 = 0.9)$, designs in figure~\ref{fig:pred-error-15-2} are easiest design among others. All the methods including OLS has small MSEP, but other methods are still dominant. In most of the situations, BayesPLS has gained minimum error with only one component. In this group, the performance of envelope is better than regular PLS and the minimum error for envelope is also achieved with fewer components.
...
Low information content in many variables has made designs in group~\ref{lst:g2} the most difficult one. In figure~\ref{fig:pred-error-40-1}, the methods based on MLE has performed poorly and often poorer than an average guess. BayesPLS and regular PLS has operated well as previous cases. Situations with steep decay of eigenvalues has caused some convergence problems for BayesPLS, especially when fitted with large number of components. However the minimum error for BayesPLS (smaller than others method) was already obtained with fewer components, so the convergence has not become a big issue. Although based on maximum likelihood, envelope has performed better than OLS estimates at its based but is very unstable over different components. 
...
Although having 40 predictors $(p\sim n)$, all methods are performing well due to rich information (high  value)
\item Based on likelihood as OLS, envelope method has outperformed OLS in every
  situations.
\item In most of the situations (except in design-16), envelope method has
  nearly attained true minimum error
\item Bayes and PLS methods are highly stable and are closer to true minimum
  error
\item Both BayesPLS and PLS has outperformed OLS and envelope methods, BayesPLS
  is able to obtain its minimum prediction error with fewer components.
\end{itemize}

\textbf{Some general points to be made in case of all designs,}

\begin{itemize}[label = $\blacktriangleright$]
\item In General regular PLS method is more stable in all situation. It is
  extensible, easy and less time consuming to fit
\item If the issue is to get closer prediction from squeezing information as
  much as possible, BayesPLS can be a immediate alternative. Its performance
  with fewer number of component is stable and better in all aspects
\item Likelihood based methods has advantage of ..., for which envelope methods
  is a best alternative among its type. Its performance is better than ordinary
  least squares even when number of predictors is nearly equals to observations.
\end{itemize}
...
% \end{equation}
%for $i^\text{th}$ estimation method

%Figure-\ref{fig:prediction-error-pls} shows that the envelope model succeeded to
%achieve smaller prediction error than PLS model. And in the case of BayesPLS method, the prediction error is smallest than other models in all the cases. For %example, in design 5, prediction error due to BayesPLS method is almost equals to the true error (0.5) present in the model which we can see in the table %presented along with the plot. This is not the case for envelope, but it was able to make closer prediction with fewer components than PLS in design 28. Similar said before (Solve)
