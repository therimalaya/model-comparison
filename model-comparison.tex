...
A comparative study was performed on these methods based on their minimum test prediction error using simulated datasets. Using model \ref{model} as a simulation model, calibration datasets were sampled from random regression framework where $\mathbf{x} \sim N\left( \mu_{\mathbf{x}}, \Sigma_{\mathbf{xx}} \right)$ with contains independent errors. Equivalently,
...
\begin{table}[ht]
  ...
  \end{tabular}
  \caption{Parameters used for simulating calibration set}
  \label{tab:parameters}
\end{table}

\bigskip

From the possible combination of above parameters, 32 calibration sets were simulated with 5 replications of each, i.e. there are 160 calibration sets ({\tt datasets}) where each 5 of them have similar properties. In order to simulate these data an r-package {\tt simrel} \citep{saebo2015simrel} was used which is based on the concept of relevant space discussed in \citet{helland1994comparison}. Simulation with {\tt simrel} follows --

\begin{itemize}[label=$\triangleright$]
\item Eigenvalues of $\boldsymbol{\Sigma}_{xx}$ are $\lambda_1, \lambda_2, \ldots, \lambda_p$. Here, we have assumed that $\lambda_1 = 1$ and $\lambda_{i+1} = e^{-\eta(j-1)}$, so that the $\eta$ parameters controls the decay of eigenvalues of $X$.
\item In equation-\ref{beta}, $\gamma_i$ can be written as,
  \begin{equation}
    \label{eq:gamma}
    \gamma_i = \frac{d_i^t \sigma_{Xy}}{\lambda_i}
  \end{equation}
  where, $d_i$ is $i^\text{th}$ eigenvector of $\boldsymbol{\Sigma}_{xx}$ and $\sigma_{Xy} = 0$ for $X$ that are relevant for $y$ and consequently, $\gamma_i \ne 0$ for $i \in \mathcal{P}$, i.e,
 \begin{equation}
...
A Comprehensive discussions about {\tt simrel} is in \citet{saebo2015simrel}. For detail explanations, we have chosen Design 5 and Design 28 for further discussions and comparison. These designs retain following properties, 
...
\begin{table}[ht]
  ...
  \begin{tabularx}{0.8\textwidth}{Xlrr}
    ...
    Decaying factor of eigenvalue           & $\eta$        & 0.5      & 0.9       \\ 
    \hline
  \end{tabularx}
...
A systematic comparison among the methods is made on the basis of their ability
to predict test samples measured by the mean square error of prediction (MSEP)
over different calibration datasets with same properties. The mean square error
of prediction (MSEP) are calculated as,
...
The test prediction error for each methods and different combination of parameters, we fit a mixed effect model, as in equation-\ref{eq:mixed-model} considering each calibration set ({\tt dataset}) to be random. In this case, we have taken the minimum MSEP that a method can give with up until 10 number of components. The mixed effect model is fitted with third order interactions between {\tt p}, {\tt R2} $(R^2)$, {\tt relpos} $(\mathcal{P})$, {\tt Method}, {\tt eta} $(\eta)$ and {\tt dataset} where each calibration sets ({\tt dataset}) were considered as a random factor. 
...
From the fitted model, we found that decay factor of eigenvalues of $\boldsymbol{\Sigma}_{xx}$ ($\eta$) and the position of relevant components ($\mathcal{P}$) are not significant along with there interactions with others. Since not all levels of interactions have significant effect on minimum MSEP, a backward elimination is performed on the complete model.
...
\begin{table}[ht]
  ...
  \begin{tabularx}{0.8\textwidth}{Xrcrr}
    ...
    \hline
  \end{tabularx}
  \caption{Analysis of Deviance Table (Type II Wald F tests with Kenward-Roger df)} 
...
\begin{figure}[!ht]
  ...
  \caption[Effect Plot]{Effect of third order interaction -- between models,
    coefficient of determination ($R^2$) and Number of variables ($p$)}
  \label{fig:effect-plot}
\end{figure}

Analysis of deviance table (Table-\ref{tab:anova-table}) for the final model we
obtained has suggesed that there is a significant third order interaction
between number of variables $(p)$, coefficient of determination $(R^2)$ and
estimation methods. The effect of this third order interaction, which we see in
figure-\ref{fig:effect-plot}, shows that the maximum likelihood based estimation
methods, in our case, envelope and ordinary least squares perform poor
prediction for noisy datasets with large number of variables. However, the
performance of envelope estimation is better as compared to ordinary least
squares also in situation of $n/p \rightarrow 1$. The plot also has suggested that BayesPLS and PLS estimation methods are better and stable as compared to other methods in all the situations. 

In order to have deeper understanding, the prediction performance of these estimation methods for each additional components included are computed. The prediction error for design 5 and design 28 for all the methods (colored) fitted with components from 0 to 10 where 0 being the null model without any components is presented in figure-\ref{fig:prediction-error}. Here, we can see that BayesPLS model has obtained the minimum prediction error with fewer components in both designs. The envelope model has performed better performance than the ordinary least squares and conventional PLS methods in case of design 28.

In addition, we compare the prediction error of these methods with conventional
PLS method by calculating the proportionate difference of prediction error as in
equation-\ref{eq:pe-pls},
